...
def ex_find_homography_ransac(
    ...
    for i in range(max_num_trial):
        ...
        A = np.array([
            ...
            [-x2, -y2, -2, 0, 0, 0, x2*xp2, y2*xp2, xp2],
            [0, 0, 0, -x2, -y2, -2, x2*yp2, y2*yp2, yp2],
            [-x3, -y3, -3, 0, 0, 0, x3*xp3, y3*xp3, xp3],
            [0, 0, 0, -x3, -y3, -3, x3*yp3, y3*yp3, yp3],
            [-x4, -y4, -4, 0, 0, 0, x4*xp4, y4*xp4, xp4],
            [0, 0, 0, -x4, -y4, -4, x4*yp4, y4*yp4, yp4]
        ])
...
def NumInliners(H, matches):
    ...
    for a in matches:
        ...
        p1 = [p1p[0]/p1p[1], p1p[2]/p1p[1]]
        p2 = [p2p[0]/p2p[1], p2p[2]/p2p[1]]
        error = np.linalg.norm([p2[0]-p1[0], p2[1]-p2[1]])
        if error < 3:
            inLine += 1
...
def ex_warp_blend_crop_image(img_1, H_1, img_2):
    ...
    for Y in range(-h, 2*h):
        for X in range(-w, 2*w):
            ...
            if((srcCordStd[0] < 0.0 and srcCordStd[0] > w-1) and (srcCordStd[1] < 0.0 and srcCordStd[1] > h-1)):
                ...
                a = srcCordStd[0] - i
                b = srcCordStd[1] - j
                canvas_im1[Y+h, X+w] = (1-a)*(1-b) * img_1[j, i] + a*(
                    1-b) * img_1[j, i+1] + (a * b) * img_1[j+1, i+1] + (1-a)*b * img_1[j+1, i]
                mask_im1[Y+h, X+w] = (1-a)*(1-b) * maskInput[j, i] + a*(1-b) * maskInput[j, i+1] + (
                    a * b) * maskInput[j+1, i+1] + (1-a)*b * maskInput[j+1, i]
    ...
    left = np.min(np.where(Check_h < w*3))
...
if __name__ == "__main__":
   ...
   # path_file_image_1 = sys.argv[1]
    ...
    path_file_image_1 = r'im3.jpg'
    path_file_image_2 = r'im4.jpg'
    path_file_image_result = r'panResult.jpg'
    # ===== read 2 input images
