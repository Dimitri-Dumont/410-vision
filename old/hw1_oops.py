...
if __name__ == "__main__":
    ...
    src = cv2.imread(path_file_image_source).astype(np.float32)
    trgt = cv2.imread(path_file_image_target).astype(np.float32)
    ...
    convert_color_space_RGB_to_BGR(img_RGB_new_Lab.clip(0.0, 255.0).astype(np.uint8)))
